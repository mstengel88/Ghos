-- Dispatch V2 caches Shopify-derived destination tax rates to avoid repeated
-- draft-order probes for the same normalized address.
--
-- This is a server-owned cache. RLS is enabled without client policies so
-- anon and authenticated Data API sessions cannot read or mutate it.
