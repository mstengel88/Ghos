...
env_file="${WINTERWATCH_STORAGE_ENV_FILE:-$repo_root/migration/supabase/secrets/winterwatch-storage.env}"
output_dir="${WINTERWATCH_STORAGE_OUTPUT:-$repo_root/migration/supabase/exports/storage/winterwatch/initial}"
expected_url="https://caegybyfdkmgjrygnavg.supabase.co"

if [[ ! -f "$env_file" ]]; then
  printf 'Private WinterWatch Storage environment file not found: %s\n' \
    "$env_file" >&2
  printf '%s\n' \
...
if [[ "${configured_url%/}" != "$expected_url" ]]; then
  printf 'Refusing unexpected WinterWatch Supabase URL in %s.\n' \
    "$env_file" >&2
  exit 1
fi
...
if [[ -z "$service_key_length" || "$service_key_length" -lt 20 ]]; then
