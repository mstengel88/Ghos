...
database_name="${LOCAL_DELIVERY_REHEARSAL_DATABASE:-}"
expected_counts_file="${LOCAL_DELIVERY_EXPECTED_COUNTS_FILE:-}"

if [[ -z "$database_name" ]]; then
  printf '%s\n' \
    'Set LOCAL_DELIVERY_REHEARSAL_DATABASE to the isolated restored database.' >&2
  exit 1
fi
...
begin
  ...
  if total_count <> 23 or valid_count <> 23 then
    raise exception
      'RLS verification failed: % of % public tables enabled; expected 23 of 23',
      valid_count,
      total_count;
  end if;
  ...
  if total_count <> 26 then
    raise exception 'Policy count mismatch: expected 26, found %', total_count;
  end if;
...
printf 'Local Delivery production restore verification passed for %s.\n' \
  "$database_name"
